...
NSString *downTableNameArray[DOWNLOADCOUNT]={@"InitUser",@"",@"",@"",@"",@"",@"",@"",@"",@"",@"",@"",@"",@"",@"",@"",@"",@"",@""};
...
- (void)startDownLoad{
    if ([WebServiceHandler isServerReachable]) {
        ...
        self.progressView=[[AlertViewWithProgressbar alloc] initWithTitle:@"同步基础数据" message:@"正在下载，请稍候……" delegate:nil cancelButtonTitle:nil otherButtonTitles:nil];
        MAINDISPATCH(^(void){
...
//            NSThread *thread = [[NSThread alloc] initWithTarget:self selector:@selector(downLoadUserInfo:) object:initUser];
//            [thread start];
