...
//    [self.textQuantity resignFirstResponder];
    ...
    if (self.textQuantity.text.doubleValue>0) {
        ...
        if (![self isUsingCustomRoadAsset]) {
            ...
            // newRoadAssetPrice.remark = @(self.customRoadAssetUnitPriceTextField.text.doubleValue);
        }
        // if([newRoadAssetPrice.remark isEmpty]){
        //     ;
        // }
        // else{
        if ((![self.textLength.text isEmpty]) && (![self.textWidth.text isEmpty])) {
            //remark=[NSString stringWithFormat:@"%@,长%@米、宽%@米",newRoadAssetPrice.remark,self.textLength.text,self.textWidth.text];
        ...
        [self.deformInfoVC addDeformationForRoadAsset:newRoadAssetPrice andQuantity:quantity withRemark:newRoadAssetPrice.spec];
    }
}
