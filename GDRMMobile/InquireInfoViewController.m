...
-(NSString*)generateCommonInquireText{
    ...
    NSString* text = [NSString stringWithFormat:@"%@%@%@",@"问：您好！我们是",organizationName,@"的路政员，现依法对您进行询问。请您如实回答我们的询问并协助调查，不得隐瞒和弄虚作假，否则将承担法律责任，您有权对有关情况作陈述和申辩，您明白吗？\n" ];
    //text = [NSString stringWithFormat:@"%@%@%@\n",text,@"答：明白，",[CaseProveInfo generateEventDescForInquire:self.caseID] ];
    text = [NSString stringWithFormat:@"%@%@\n",text,@"答：明白。" ];
    text = [NSString stringWithFormat:@"%@%@%@。\n",text,@"问：您是由于什么原因发生交通事故的？\n答：因", [CaseInfo caseInfoForID:self.caseID].case_type];
    text = [NSString stringWithFormat:@"%@%@",text,@"问：事故是否造成人员伤亡，车辆是否造成损坏及损坏程度如何？\n答：" ];
     
    Citizen *currentcitizen= [Citizen citizenByCaseID:self.caseID andNexus:self.textNexus.text];
        text = [NSString stringWithFormat:@"%@%@车辆%@损坏。\n",text,[CaseProveInfo generateWoundDesc:self.caseID] ,currentcitizen.bad_desc];
    //text = [NSString stringWithFormat:@"%@问：%@\n",text,[CaseProveInfo generateDefaultPayReason:self.caseID] ];
    text = [NSString stringWithFormat:@"%@问：事故造成公路路产损坏，损坏路产详见公路赔（补）偿清单。您是否认可？\n",text ];
    text = [NSString stringWithFormat:@"%@%@",text,@"答：认可。\n问：以上内容是否属实？\n答：属实。" ];
    return text;
}
